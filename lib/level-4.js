 ...
 */
  export function canMakeAmount(target, drawer) {
    ...
    //as usual go through the draw with loop rember to put the ++ right 
    for (let i = 0; i < drawer.length; i++) {
      ...
      if (maxAmountFromItem <= amountNeeded) { // dont forget the { !!!!! That gave you a sintax error !!
        ...
        currentAmount += amountWeCanTake; //if  max*** is less than or equal too amount needed then add current ammount to max 
      }
    ...
    return false; // We cannot make the target amount so return a false 
